...
class SimpleGenerator_prediction(tensorflow.keras.utils.Sequence):
    ...
    def batch_generation(self):
        ...
        #if input_batch.shape[1] == 31901:
            #input_batch = input_batch[:, :31900]
            #input_batch = input_batch[:, :4,:]
        #if input_batch.shape[1] == 7976:
            #input_batch = input_batch[:, :7975]
...
class RandomGenerator(tensorflow.keras.utils.Sequence):
    def __init__(self,
        ...
        self.sound_len = 95700
        self.spike_len = 23925
    ...
    def batch_generation(self):
        if self.type_generator == 'spk2snd':
            input_batch = np.random.rand(self.batch_size, self.spike_len)
            output_batch = np.random.rand(self.batch_size, self.sound_len)
        elif self.type_generator == 'snd2snd':
            input_batch = np.random.rand(self.batch_size, self.sound_len)
            output_batch = np.random.rand(self.batch_size, self.sound_len)
        else:
