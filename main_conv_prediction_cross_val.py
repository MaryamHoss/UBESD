...
from TrialsOfNeuralVocalRecon.data_processing.data_collection import getData
...
def cfg():
    ...
    fusion_type = '_FiLM_v1_orthogonal_unet_convblock:cnrd_mmfilter:64:64_dilation:_nconvs:4'
    fusion_type='_FiLM_v1_orthogonal_unet_convblock:cnrd_mmfilter:64:64_dilation:_nconvs:4_voice_preprocessing'
...
def get_callbacks(path_best_model, history_path, data_type, images_dir):
    checkpoint = ModelCheckpoint(path_best_model, monitor='loss', verbose=1, save_best_only=True)
    #save_weights_only=True)
    earlystopping = EarlyStopping(monitor='loss', mode='min', verbose=1, patience=10)
    reduce_lr = ReduceLROnPlateau(monitor='loss', patience=5, verbose=1, cooldown=1)
    csvlogger = tf.keras.callbacks.CSVLogger(history_path),

    callbacks = [
        checkpoint, earlystopping, csvlogger, reduce_lr,
        TimeStopping(115200, 1)  # 31 hours: 111600 s 21 hours: 75600 s #223200 62 hours #32: 115200
    ]  # , tensorboard]
...
def main(exp_type, data_type,
         ...
         sound_len_test, spike_len_test, batch_start, batch_stop, batch_step, seed,cross_number):
    ...
    if 'voice_preprocessing' in data_type:
        ...
        callbacks = get_callbacks(path_best_model, history_path, data_type, images_dir)
    ...
    del callbacks
    del generators['train'], generators['val'], generators['test']
