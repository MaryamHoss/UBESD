...
class Decoder(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=None):
        ...
        for i in range(self.num_layers):
            decoder_output, block1, block2 = self.dec_layers[i]([x, enc_outputs[i]])
...
class TransformerParallel(tf.keras.layers.Layer):
    def __init__(self, num_layers, d_model, num_heads, dff,
        ...
        self.encoder = Encoder(num_layers, d_model, num_heads, dff,
                               pe_input, rate, kernel_initializer=kernel_initializer)

        self.decoder = Decoder(num_layers, d_model, num_heads, dff,
                               pe_target, rate, kernel_initializer=kernel_initializer)
    ...
    def call(self, inputs, training=None):
        ...
        dec_outputs, attention_weights = self.decoder([tar, enc_output])

        dec_output = tf.keras.layers.Add()(dec_outputs)
